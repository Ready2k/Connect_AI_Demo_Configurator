...
export default function ExperiencePage() {
  ...
  type LoopLogStatus = "info" | "ok" | "warn" | "error" | "stopped";
  interface LoopLogEntry { text: string; status: LoopLogStatus }
  ...
  // Resolve agent context from the selected entry agent name
  const resolveAgentContext = (entryAgentName: string) => {
    const agent = projectConfig.agents.find((a) => a.name === entryAgentName);
  ...
  const handleGenerate = async (feedback?: { issues: unknown[]; suggestions: unknown[]; previousFlowJson: string }) => {
    ...
    updateExperience(activeExperienceId, {
      ...
      generatedFlowJson: undefined,
      verificationResult: undefined,
    });
    addLog("INFO", "ExperiencePage", feedback ? "Starting flow regeneration with feedback" : "Starting flow generation", {
    ...
    try {
      const agentContext = resolveAgentContext(activeExperience.journeyConfig.entryAgentName);
  ...
  const handleAutoLoop = async () => {
    ...
    const modelId = projectConfig.aws.flowAssistantModelId;
    const region = projectConfig.aws.connectRegion;
    const agentContext = resolveAgentContext(journeyConfig.entryAgentName);
    ...
    const log = (text: string, status: LoopLogStatus) =>
      setLoopLog((prev) => [...prev, { text, status }]);

    // Convergence tracking — stop if issue count doesn't improve for 2 consecutive iterations
    let lastIssueCount = Infinity;
    let nonImprovingStreak = 0;
    const MAX_NON_IMPROVING = 2;

    // Seed initial feedback from any existing verification result with issues
    type Feedback = { issues: unknown[]; suggestions: unknown[]; previousFlowJson: string };
    let feedback: Feedback | undefined =
      ...
      activeExperience.generatedFlowJson
        ? {
            issues: activeExperience.verificationResult.issues,
            suggestions: activeExperience.verificationResult.suggestions,
            previousFlowJson: activeExperience.generatedFlowJson,
          }
        : undefined;

    for (let i = 0; i < MAX_LOOP_ITERATIONS; i++) {
      if (shouldStopRef.current) { log("Loop stopped by user.", "stopped"); break; }

      setLoopIteration(i + 1);
      log(`Iteration ${i + 1} of ${MAX_LOOP_ITERATIONS}: generating flow${feedback ? " with feedback" : ""}…`, "info");
      ...
      try {
        const res = await fetch("/api/experience/generate", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            journeyConfig, library, modelId, region,
            ...(agentContext ? { agentContext } : {}),
            ...(feedback ? { verificationFeedback: feedback } : {}),
          }),
        });
        const data = await res.json();
        if (Array.isArray(data.logs)) {
          for (const e of data.logs) addLog(e.level, "AutoLoop", e.message, e.details);
        }
        if (data.status === "success" && data.flowJson) {
          flowJson = data.flowJson;
          updateExperience(activeExperienceId, { generationStatus: "success", generatedFlowJson: flowJson });
          log(`Iteration ${i + 1}: flow generated.`, "ok");
        } else {
          log(`Iteration ${i + 1}: generation failed — ${data.error ?? "unknown"}`, "error");
          updateExperience(activeExperienceId, { generationStatus: "manual_review", generationError: data.error });
          break;
        }
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : "Generation failed";
        log(`Iteration ${i + 1}: generation error — ${msg}`, "error");
        updateExperience(activeExperienceId, { generationStatus: "manual_review", generationError: msg });
        break;
      }

      if (shouldStopRef.current) { log("Loop stopped by user.", "stopped"); break; }

      log(`Iteration ${i + 1}: verifying…`, "info");
      try {
        const res = await fetch("/api/experience/verify", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ journeyConfig, flowJson, modelId, region }),
        });
        const verifyData = await res.json();
        updateExperience(activeExperienceId, { verificationResult: verifyData });

        const issueCount: number = Array.isArray(verifyData.issues) ? verifyData.issues.length : 0;

        if (issueCount === 0) {
          log(`Iteration ${i + 1}: no issues found — flow is correct.`, "ok");
          break;
        }

        log(`Iteration ${i + 1}: ${issueCount} issue${issueCount !== 1 ? "s" : ""} found.`, "warn");

        // Convergence check — if issue count isn't reducing, stop early
        if (issueCount >= lastIssueCount) {
          nonImprovingStreak++;
          if (nonImprovingStreak >= MAX_NON_IMPROVING) {
            log(
              `Issue count not reducing after ${MAX_NON_IMPROVING} attempts (${lastIssueCount} → ${issueCount}). ` +
              `Remaining issues may be architectural — review manually or adjust the Journey Configuration.`,
              "warn"
            );
            break;
          }
          log(`Issue count unchanged (${issueCount}) — trying once more with stronger feedback…`, "info");
        } else {
        ...
        if (i === MAX_LOOP_ITERATIONS - 1) {
          log(`Reached maximum of ${MAX_LOOP_ITERATIONS} iterations. Review remaining issues manually.`, "warn");
          break;
        }

        feedback = { issues: verifyData.issues, suggestions: verifyData.suggestions, previousFlowJson: flowJson! };
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : "Verification failed";
        log(`Iteration ${i + 1}: verification error — ${msg}`, "error");
        break;
      }
    }
    ...
    reader.onload = (event) => {
      try {
        ...
        if (activeExperienceId) {
          updateExperience(activeExperienceId, {
            generatedFlowJson: content,
            generationStatus: "success",
            generationError: undefined,
            lastGeneratedAt: new Date().toISOString(),
          });
          addLog("SUCCESS", "ExperiencePage", "Flow JSON uploaded successfully", { fileName: file.name, length: content.length });
        }
      } catch (err) {
        const msg = err instanceof Error ? err.message : "Invalid JSON";
        addLog("ERROR", "ExperiencePage", "Flow upload failed", { error: msg });
        alert("Invalid JSON file: " + msg);
      }
    };
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={generating || autoLooping}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 disabled:opacity-50 transition-colors text-sm font-medium"
                >
                  {generating && !autoLooping ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                  {generating && !autoLooping ? "Generating..." : "Generate Flow"}
                </button>
                <button
                  onClick={handleAutoLoop}
                  disabled={generating || autoLooping || verifying}
                  title={`Auto-generate → verify → fix up to ${MAX_LOOP_ITERATIONS} times until no issues remain`}
                  className="flex items-center justify-center gap-1.5 px-3 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50 transition-colors text-sm font-medium whitespace-nowrap"
                >
                  {autoLooping ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>↺</span>}
                  {autoLooping ? `Loop ${loopIteration}/${MAX_LOOP_ITERATIONS}` : "Auto-fix Loop"}
                </button>
              </div>
              ...
              {activeExperience.generationStatus === "manual_review" && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3 space-y-2">
                  ...
                  {activeExperience.generationError?.toLowerCase().includes("model identifier") && (
                    <p className="text-xs text-yellow-600">
                      The Flow Assistant Model ID is invalid for this region. Go to{" "}
                      <a href="/settings" className="underline font-medium">Settings → Flow Assistant Model</a>{" "}
                      and click <strong>Test model</strong> to find a working model ID.
                    </p>
                  )}
                  {activeExperience.generatedFlowJson && (
                    <button
              ...
              {activeExperience.generationStatus === "success" && !autoLooping && (
              ...
              {/* Loop progress panel */}
              {(autoLooping || loopDone) && loopLog.length > 0 && (
                <div className="border border-indigo-200 rounded-md bg-indigo-50 p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      {autoLooping && <Loader2 className="w-3.5 h-3.5 animate-spin text-indigo-600" />}
                      <span className="text-xs font-semibold text-indigo-800">
                        {autoLooping
                          ? `Auto-fix Loop — iteration ${loopIteration} of ${MAX_LOOP_ITERATIONS}`
                    ...
                    <div className="flex items-center gap-2">
                      {autoLooping && (
                        <button
                          ...
                          className="px-2 py-0.5 text-xs font-medium text-red-600 bg-red-50 border border-red-200 rounded hover:bg-red-100 transition-colors"
                        >
                          Stop
                        </button>
                      )}
                      {loopDone && !autoLooping && (
                        <button
                          onClick={() => { setLoopDone(false); setLoopLog([]); setLoopIteration(0); }}
                    ...
                    {loopLog.map((entry, i) => (
                      <div
                        ...
                        className={`text-xs flex items-start gap-1.5 ${
                          ...
                          entry.status === "warn"    ? "text-amber-700" :
                          entry.status === "error"   ? "text-red-700"   :
                          entry.status === "stopped" ? "text-gray-500"  :
                          "text-indigo-700"
                        }`}
                      >
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-4">
                ...
                {verifyOpen && (
                  <div className="mt-3 space-y-3">
                    ...
                    {activeExperience.verificationResult && activeExperience.generatedFlowJson && (
                      <button
                        onClick={() => handleGenerate({
                          issues: activeExperience.verificationResult!.issues,
                          suggestions: activeExperience.verificationResult!.suggestions,
                        ...
                        disabled={generating || verifying || autoLooping}
                          ...
                          <>
                            ...
                            <ul className="text-xs text-red-600 list-disc list-inside space-y-0.5">
                              {activeExperience.verificationResult.issues.map((issue, i) => (
                                <li key={i}>{itemToString(issue)}</li>
                              ))}
                            </ul>
                          </>
                        )}
            ...
            <div className="space-y-2">
              <div className="flex gap-3 flex-wrap">
                <button
                  ...
                  disabled={generating || autoLooping}
                  className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 disabled:opacity-50 transition-colors text-sm font-medium"
                >
                  Upload JSON
                </button>
                {activeExperience.generatedFlowJson && (
              ...
              {activeExperience.generatedFlowJson && publishResult && (
              ...
              {activeExperience.generatedFlowJson && publishError && (
